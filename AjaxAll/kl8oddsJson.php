...
if ($tid == 1)
{
	...
	$sql = "SELECT g_qishu,g_ball_1, g_ball_2, g_ball_3, g_ball_4, g_ball_5, g_ball_6, g_ball_7, g_ball_8, g_ball_9, g_ball_10,g_ball_11, g_ball_12, g_ball_13, g_ball_14, g_ball_15, g_ball_16, g_ball_17, g_ball_18, g_ball_19, g_ball_20 FROM `g_history8` WHERE {$date} and g_ball_1 is not null ORDER BY g_qishu ASC";
	...
	if($len>=1)
	{
		...
		for($k=1;$k<$len;$k++)
		{
			...
			{
			$row2.='<br><span style="color:red;">'.$numArray1[$k][1].'</span>';
			  }else if($ball=="虎" || $ball=="單" || $ball=="小")
				{
				$row2.='<br><span style="color:#009100;">'.$numArray1[$k][1].'</span>';
				 }else{
			...
			if($ball=="龍" || $ball=="雙" || $ball=="大")
			{
			$row2.="</td>,<td valign=top class='z_cl'><span style='color:red;'>".$numArray1[$k][1].'</span>';
			 
			  }else if($ball=="虎" || $ball=="單" || $ball=="小")
				{
				$row2.="</td>,<td valign=top class='z_cl'><span style='color:#009100;'>".$numArray1[$k][1].'</span>';
				 }else{
			...
			{
			$row3.='<br><span style="color:red;">'.$numArray1[$k][2].'</span>';
			  }else if($ball2=="虎" || $ball2=="單" || $ball2=="小")
				{
				$row3.='<br><span style="color:#009100;">'.$numArray1[$k][2].'</span>';
				 }else{
			...
			if($ball2=="龍" || $ball2=="雙" || $ball2=="大")
			{
			$row3.="</td>,<td valign=top class='z_cl'><span style='color:red;'>".$numArray1[$k][2].'</span>';
			 
			  }else if($ball2=="虎" || $ball2=="單" || $ball2=="小")
				{
				$row3.="</td>,<td valign=top class='z_cl'><span style='color:#009100;'>".$numArray1[$k][2].'</span>';
				 }else{
			...
			{
			$row5.='<br><span style="color:red;">'.$numArray1[$k][4].'</span>';
			  }else if($ball4=="虎" || $ball4=="單" || $ball4=="小" || $ball4=="後")
				{
				$row5.='<br><span style="color:#009100;">'.$numArray1[$k][4].'</span>';
				 }else{
			...
			if($ball4=="龍" || $ball4=="雙" || $ball4=="大" || $ball4=="前")
			{
			$row5.="</td>,<td valign=top class='z_cl'><span style='color:red;'>".$numArray1[$k][4].'</span>';
			 
			  }else if($ball4=="虎" || $ball4=="單" || $ball4=="小" || $ball4=="後")
				{
				$row5.="</td>,<td valign=top class='z_cl'><span style='color:#009100;'>".$numArray1[$k][4].'</span>';
				 }else{
			...
			{
			$row6.='<br><span style="color:red;">'.$numArray1[$k][5].'</span>';
			  }else if($ball5=="虎" || $ball5=="單" || $ball5=="小")
				{
				$row6.='<br><span style="color:#009100;">'.$numArray1[$k][5].'</span>';
				 }else{
			...
			if($ball5=="龍" || $ball5=="雙" || $ball5=="大")
			{
			$row6.="</td>,<td valign=top class='z_cl'><span style='color:red;'>".$numArray1[$k][5].'</span>';
			 
			  }else if($ball5=="虎" || $ball5=="單" || $ball5=="小")
				{
				$row6.="</td>,<td valign=top class='z_cl'><span style='color:#009100;'>".$numArray1[$k][5].'</span>';
				 }else{
