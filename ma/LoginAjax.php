...
if ($_SERVER["REQUEST_METHOD"] == 'POST')
{
	...
	if ($_POST['ValidateCode'] == $_SESSION['VCODE'])
	{
		...
		$db = new DB();
		$sql = "SELECT * FROM `j_manage` WHERE `g_name` = '{$loginName}' AND `g_password` = '{$loginPwd}' LIMIT 1 ";
		...
		if ($result)
		{
			...
			$sql = "UPDATE `j_manage` SET `g_uid` = '{$uniqid}', `g_ip` = '{$loginIp}', `g_date` = '{$loginDate}', `g_out` =1, `g_count_time`=now(),`g_state` =1 WHERE `g_name` = '{$loginName}' AND `g_password` = '{$loginPwd}' ";
