...
//$resulth = $db->query("SELECT g_auto, g_gd FROM j_manage where g_name='{$gmname}'  ORDER BY g_id DESC", 1);
	
$userModel = new UserModel();
$RankList = $userModel->GetRankAlldl($Users[0]['g_nid']);
$MemberList = $userModel->GetMemberAlldl($Users[0]['g_nid']);
...
if ($_GET['start']!="" and $_GET['end']!=""){
	...
	$total = $db->query("SELECT `g_id` FROM `g_zhudan` WHERE {$date}  {$gp}  {$rzz}  {$r}  {$rn} {$gn} and g_s_nid like'{$Users[0]['g_nid']}%'", 3);
	
	$page = new Page($total, $pageNum);
	$sql = "SELECT * FROM g_zhudan WHERE {$date}  {$gp}  {$r}  {$rn}  {$rzz} {$gn} and g_s_nid like'{$Users[0]['g_nid']}%' ORDER BY g_id DESC {$page->limit} ";
...
<script type="text/javascript" src="/Admin/temp/js/dlzhudan.js"></script>
<title></title>
<script>
function setyc(zdid,title)
	{
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data==1){
				$("#"+zdid).html("<font color=\"red\">不隐藏</font>");
				$("#"+zdid).attr("title","no");
				 $("#fail"+zdid).html("<font color=\"red\">不隐藏</font>");
...
if ($peizhijxssc == "1") {
    ...
    echo "<option " . $lie3 . "  value=\"3\">全天五分彩</option>";
} ?>
	   <?php
if ($peizhixjssc == "1") {
    if ($GameType == 10) {
...
if ($peizhixyft == "1") {
    ...
    echo " <option " . $lie4 . "  value=\"4\">幸運飛艇</option>";
} ?>
		<?php
if ($peizhipk10 == "1") {
    if ($GameType == 6) {
    ...
    echo " <option " . $lie6 . " value=\"6\">北京賽車(PK10)</option>";
} ?>
		 <?php
if ($peizhijssz == "1") {
    if ($GameType == 7) {
    ...
    echo " <option " . $lie7 . "  value=\"7\">江苏骰寶(快3)</option>";
} ?>
		 <?php
if ($peizhikl8 == "1") {
    if ($GameType == 8) {
    ...
    echo "  <option " . $lie8 . "  value=\"8\">快樂8(雙盤)</option>";
} ?>
		 <?php
if ($peizhinc == "1") {
    if ($GameType == 9) {
    ...
    echo "  <option " . $lie9 . "  value=\"9\">重慶幸運農場</option>";
} ?>  				
                  </select></td>
          ...
          <td class="c"><!-- strat -->
            ...
            <table border="0" cellspacing="0" class="conter">
              ...
              <tr align="center" onmouseover="this.style.backgroundColor='#FFFFA2'" onmouseout="this.style.backgroundColor=''">
				...
				<td><?php echo $result[$i]['mxip'];
				
				//对上级进行对比 如果返回有数据则是有 
				$ggnid=substr($result[$i]['g_s_nid'],0,32*2);
			...
			$ggnid=substr($result[$i]['g_s_nid'],0,32*3);
			 $result21 = $db->query("select g_out,g_name from g_rank  where g_nid='{$ggnid}'", 1);
			$shangji= $result21[0]['g_name'];
			$gudongcha=$shangji;
			//echo $dailicha;
			//判断上级总代理
             $ggnid=substr($result[$i]['g_s_nid'],0,32*4);
			...
			//echo $dailicha;
			//判断上级代理
             $ggnid=$result[$i]['g_s_nid'];
			 $result21 = $db->query("select g_out,g_name from g_rank  where g_nid='{$ggnid}'", 1);
			$shangji= $result21[0]['g_name'];
			$dailicha=$shangji;
			//echo $dailicha;
									$total3228 = $db->query("SELECT * FROM `g_login_log` WHERE g_ip = '{$result[$i]['mxip']}' and g_name='".$gongsicha."' and g_name != '".$result[$i]['g_name']."' ", 1);
									$total32282 = $db->query("SELECT * FROM `g_login_log` WHERE g_ip = '{$result[$i]['mxip']}' and g_name='".$gudongcha."' and g_name != '".$result[$i]['g_name']."' ", 1);
									$total32283 = $db->query("SELECT * FROM `g_login_log` WHERE g_ip = '{$result[$i]['mxip']}' and g_name='".$zongdaicha."' and g_name != '".$result[$i]['g_name']."' ", 1);
									$total32284 = $db->query("SELECT * FROM `g_login_log` WHERE g_ip = '{$result[$i]['mxip']}' and g_name='".$dailicha."' and g_name != '".$result[$i]['g_name']."' ", 1);
									 echo $result[$i]['g_ip'];
                ...
                <td><table border="0" cellspacing="0" cellpadding="0">
                    <tr>
                     ...
                     -->
					  <td class="nones" width="40"><a id='<?php echo $result[$i]['g_id']?>' title="<?php echo $result[$i]['yinchang']==1? 'no':'yes'?>" href="javascript:void(0)" onclick="setyc(<?php echo $result[$i]['g_id']?>,this.title)"><?php echo $result[$i]['yincang']==1? '<font color="red">不隐藏</font>':'隐藏'?></a></td>
                      ...
                      <?php if ($GameType == 0){
									 if($result[$i]['g_type']=="廣東快樂十分"){
										...
										}elseif($result[$i]['g_type']=="北京賽車(PK10)"){
										$links = 'UpCrystalpk.php';
										}elseif($result[$i]['g_type']=="幸運飛艇"){
										$links = 'UpCrystalxyft.php';
										}elseif($result[$i]['g_type']=="江苏骰寶(快3)"){
										$links = 'UpCrystaljs.php';
										}elseif($result[$i]['g_type']=="快樂8(雙盤)"){
										$links = 'UpCrystalkl8.php';
										}elseif($result[$i]['g_type']=="重慶幸運農場"){
