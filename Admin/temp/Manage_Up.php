...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['cid']))
{
	...
	$s_panl=strtoupper($s_pan[0]);
     ...
     if (!isset($_POST['s_pan'])) exit(back('请选择会员所属盘口！'));
	...
	//	if (!Matchs::isNumber($user_lock)) exit(back('帳號限額輸入錯誤！'));
		...
		//判断是不是现金会员
		if($memberModel[0]['g_leixing']==1){
		//exit;
		if ($s_money != $g_money_yes){
			if ($s_money > $g_money_yes){//输入的值和数据库原值对比
	       ...
	       $valueList = array();
			...
			$valueList['g_initial_value'] = $g_money_yes2;
			$valueList['g_up_value'] = $s_money;
			$valueList['g_up_type'] = '余额';
			$valueList['g_s_id'] = 1;
			$valueList['g_nid'] = $nid;
          ...
          <tr>
            ...
            <td class="c"><!-- strat -->
              <table border="0" cellspacing="0" class="conter">
                ...
                <tr style="height:28px">
                  <td class="bj"><?php if($memberModel[0]['g_leixing']==0){ echo "信用額度";}else{ echo "可用余额";}?></td>
                  <td nowrap="nowrap" class="left_p5" ><input style="width:85px;" class='inp1MM' name="s_money" id="s_money"  onkeyup="DX(this.value);"  value="<?php if($memberModel[0]['g_leixing']==0){ echo $memberModel[0]['g_money']; }else { echo $memberModel[0]['g_money_yes']; }?>"  />
                  &nbsp;              
  <span class="red" style="font-weight:bold" name="rmb" id="rmb"><?php echo $memberModel[0]['money_dx']?></span><?php if($memberModel[0]['g_leixing']==0){ echo "<font color=\"344b50\"> 『&nbsp;可‘回收’餘額&nbsp;<span id='money_ky' >".$validMoneys."</span>&nbsp;』</font>";}?></td><input  type="hidden"  id='money_dx' name='money_dx' value="" />
                </tr>
                ...
                <tr style="height:28px">
                  ...
                  <td class="left_p5">
                    ...
                    if(strstr($p,'A')!=''|| strstr($p,'a')!=''){echo ' <input type="radio" value="a" name="s_pan[]"  '.$a.' onclick="check(this)" /> A盤&nbsp;';} 
                    
                    if(strstr($p,'B')!=''|| strstr($p,'b')!=''){echo ' <input type="radio" value="b" name="s_pan[]" '.$b.' onclick="check(this)" /> B盤&nbsp;';} 
                    
                    if(strstr($p,'C')!=''|| strstr($p,'c')!=''){echo '<input type="radio" value="c" name="s_pan[]" '.$c.' onclick="check(this)" /> C盤&nbsp; ';}
					}else{
					
					echo ' 
                    <input type="radio" value="a" name="s_pan[]"   onclick="check(this)" />
                    A盤&nbsp;
                    <input type="radio" value="b" name="s_pan[]"  onclick="check(this)" />
                    B盤&nbsp;
                    <input type="radio" value="c" name="s_pan[]"  onclick="check(this)" />
                    C盤&nbsp;';
