...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['cid']))
{
	...
	$g_panlus = $_POST['g_panlus']; //所属盘口abc
	$jishizhudan = $_POST['jishizhudan'];
	$zaixiantongji = $_POST['zaixiantongji'];
	$duanxiaoxi = $_POST['duanxiaoxi'];
	$touzhuguanli = $_POST['touzhuguanli'];
	$lishirizhi = $_POST['lishirizhi'];
	 
	//if (!Matchs::isStringChi($s_F_Name, 2, 8)) exit(back('輸入名稱錯誤！'));
	if (isset($_POST['s_panlus'])) exit(back('请选择会员所属盘口！'));
	...
	if ($userList) {
		...
		if ($cid == 1 && $LoginId == 89){
			...
			$sql = " UPDATE `g_rank` SET `jishizhudan` = '{$jishizhudan}',`zaixiantongji` = '{$zaixiantongji}',`duanxiaoxi` = '{$duanxiaoxi}',`touzhuguanli` = '{$touzhuguanli}',`lishirizhi` = '{$lishirizhi}',`g_f_name` = '{$s_F_Name}', `g_panlus` = '{$g_panlus}',g_password = '{$s_Pwd}',`g_zy`='{$g_zy}', `g_lock` = '{$lock}' ".$g_pwd.",g_money='{$s_money}',g_distribution='{$s_next_ky}',g_distribution_limit='{$s_next_ky}' WHERE `g_name` = '{$userList[0]['g_name']}' LIMIT 1";
          ...
          //  dump($userList[0]['g_login_id']);
			...
			`g_panlus`='{$g_panlus}',
			`jishizhudan`='{$jishizhudan}',
			`zaixiantongji`='{$zaixiantongji}',
			`duanxiaoxi`='{$duanxiaoxi}',
			`touzhuguanli`='{$touzhuguanli}',
			`lishirizhi`='{$lishirizhi}',
			`g_lock`='{$lock}' ".$g_pwd."
            	...
            	<table border="0" cellspacing="0" class="main">
                  ...
                  <script type="text/javascript"> 
										function check(spanl){
   											var flag=0;
   										for(var i=0;i<document.getElementsByName("g_panlus").length;i++){
       										if(document.getElementsByName("g_panlus")[i].checked==true){
       											flag++;
    										}
   										}
  											 if(flag==0){
                  ...
                  <td class="left_p5"> 
					...
					if(strstr($P,'abc')!=''){echo ' <input type="radio" value="abc" name="g_panlus"  '.$abc.' onclick="check(this)" />所有盤&nbsp;';} 
 
                    if(strstr($P,'a')!=''){echo ' <input type="radio" value="a" name="g_panlus"  '.$a.' onclick="check(this)" /> A盤&nbsp;';} 
                    
                      if(strstr($P,'b')!=''){echo ' <input type="radio" value="b" name="g_panlus" '.$b.' onclick="check(this)" /> B盤&nbsp;';} 
                    
                    if(strstr($P,'c')!=''){echo '<input type="radio" value="c" name="g_panlus" '.$c.' onclick="check(this)" /> C盤&nbsp; ';}
					...
					echo '<input type="radio" value="abc" name="g_panlus" checked="checked" onclick="check(this)" />
                    所有盤&nbsp;
                    <input type="radio" value="a" name="g_panlus"   onclick="check(this)" />
                    A盤&nbsp;
                    <input type="radio" value="b" name="g_panlus"  onclick="check(this)" />
                    B盤&nbsp;
                    <input type="radio" value="c" name="g_panlus"  onclick="check(this)" />
                    C盤&nbsp;';
				 ...
				 <tr style="height:28px">
				             	...
				             	{
								 //判断此用户有没有开启
								if($userList[0]['jishizhudan']=="on")
								{
								$jishizhudan22='checked="checked"';
								}
								if($userList[0]['zaixiantongji']=="on")
								{
								$zaixiantongji22='checked="checked"';
								}
								if($userList[0]['duanxiaoxi']=="on")
								{
								$duanxiaoxi22='checked="checked"';
								}
								if($userList[0]['touzhuguanli']=="on")
								{
								$touzhuguanli22='checked="checked"';
								}
								if($userList[0]['lishirizhi']=="on")
								{
								$lishirizhi22='checked="checked"';
								}
								//-----------------------------------------------
								//判断是不是开启
								if($Users[0]['jishizhudan']=="on")
								{
								echo "即时注单：<input type=\"checkbox\" name=\"jishizhudan\" ".$jishizhudan22.">&nbsp; ";
								}
								if($Users[0]['zaixiantongji']=="on")
								{
								echo" 在线统计：<input type=\"checkbox\" name=\"zaixiantongji\" ".$zaixiantongji22.">&nbsp; ";
								}
								if($Users[0]['duanxiaoxi']=="on")
								{
								echo"	短消息：<input type=\"checkbox\" name=\"duanxiaoxi\" ".$duanxiaoxi22.">&nbsp; ";
								}
								if($Users[0]['touzhuguanli']=="on")
								{
								echo"	投注管理：<input type=\"checkbox\" name=\"touzhuguanli\" ".$touzhuguanli22.">&nbsp;";
								}if($Users[0]['lishirizhi']=="on")
								{
									echo"历史日志：<input type=\"checkbox\" name=\"lishirizhi\" ".$lishirizhi22.">&nbsp;";
								}
								...
								echo	"即时注单：<input type=\"checkbox\" name=\"jishizhudan\" ".$jishizhudan22.">&nbsp; 
									在线统计：<input type=\"checkbox\" name=\"zaixiantongji\" ".$zaixiantongji22." >&nbsp; 
									短消息：<input type=\"checkbox\" name=\"duanxiaoxi\" ".$duanxiaoxi22.">&nbsp; 
									投注管理：<input type=\"checkbox\" name=\"touzhuguanli\" ".$touzhuguanli22.">&nbsp;
									历史日志：<input type=\"checkbox\" name=\"lishirizhi\" ".$lishirizhi22.">&nbsp;";
