...
markPos("后台-浏览报表");
	...
	$total = $db->query("SELECT g_id FROM g_zhudan WHERE {$date} AND yincang < 1 AND g_nid = '{$s_name}'  {$vvv}  ", 3);
    $pageNum = 100;
    $page = new Page($total, $pageNum);
    $zhudan = $db->query("SELECT * FROM `g_zhudan` WHERE  {$date} AND yincang < 1 AND  g_nid = '{$s_name}'  {$vvv} ORDER BY g_id DESC {$page->limit} ", 1);	
