...
function sumzc($result, $loginid)
{
	...
	if ($loginid == 89 || $loginid == 56){
		...
		$c[1] = ($result['g_distribution_3']/100) * ((($result['g_tueishui_3']/100))*$j);
	} else if ($loginid == 22){
		$s = $result['g_tueishui_2'] >0 ? ($result['g_tueishui_2']/100) : ($result['g_tueishui']/100);
		...
		$s = $result['g_tueishui_1'] >0 ? ($result['g_tueishui_1']/100) : ($result['g_tueishui']/100);
		...
		if ($result['g_distribution'] > 0){
			...
			$c[1] = ($result['g_distribution']/100) * ((($result['g_tueishui_3']/100))*$j);
		} else {
			$c[0] =$j;
			$c[1] =(($result['g_tueishui_3']/100))*$j;
		}
	}
	return $c;
}
