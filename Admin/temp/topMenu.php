...
Link2[9] =
 ...
 <?php }?>
  <?php if (!isset($Users[0]['g_lock_1_1'])&& $cz=="1"){?>
	 ...
	 '<a href="cz.php" target="'+target+'">会员冲值</a>'+
...
function SubonCK(v){
	...
	$("#currentType").html("北京賽車(PK10)");
	}else if(v==72){
	$("#currentType").html("江苏鱼虾蟹");
	}else if(v==7){
	$("#currentType").html("江苏骰寶(快3)");
	}else if(v==8){
	$("#currentType").html("快樂8(雙盤)");
	}else if(v==3){$("#currentType").html("全天五分彩");}
	else if(v==4){$("#currentType").html("幸運飛艇");}
	else if(v==9){$("#currentType").html("重慶幸運農場");}
...
function SubonCK2(v){
	if(v<10){
		...
		if(v==11){
			...
			{parent.frames.mainFrame.location.href="_Result.php";$("#navListBox").html("");}
			if($("#currentType").text()=='廣東快樂十分')
			{parent.frames.mainFrame.location.href="_Result.php";$("#navListBox").html("");}
			if($("#currentType").text()=='重慶時時彩')
			{parent.frames.mainFrame.location.href="_Result.php?id=2";$("#navListBox").html("");}
			if($("#currentType").text()=='北京賽車(PK10)')
			{parent.frames.mainFrame.location.href="_Result.php?id=6";$("#navListBox").html("");}
			if($("#currentType").text()=='江苏鱼虾蟹')
			{parent.frames.mainFrame.location.href="_Result.php?id=7";$("#navListBox").html("");}
			if($("#currentType").text()=='江苏骰寶(快3)')
			{parent.frames.mainFrame.location.href="_Result.php?id=7";$("#navListBox").html("");}
			if($("#currentType").text()=='快樂8(雙盤)')
			{parent.frames.mainFrame.location.href="_Result.php?id=8";$("#navListBox").html("");}
			if($("#currentType").text()=='全天五分彩')
			{parent.frames.mainFrame.location.href="_Result.php?id=3";$("#navListBox").html("");}
			if($("#currentType").text()=='广东十一选五')
			{parent.frames.mainFrame.location.href="_Result.php?id=5";$("#navListBox").html("");}
			if($("#currentType").text()=='幸運飛艇')
			{parent.frames.mainFrame.location.href="_Result.php?id=4";$("#navListBox").html("");}
			if($("#currentType").text()=='重慶幸運農場')
			{parent.frames.mainFrame.location.href="_Result.php?id=9";$("#navListBox").html("");}
			if($("#currentType").text()=='新疆时时彩')
			{parent.frames.mainFrame.location.href="_Result.php?id=10";$("#navListBox").html("");}
			if($("#currentType").text()=='天津时时彩')
			{parent.frames.mainFrame.location.href="_Result.php?id=11";$("#navListBox").html("");}
			}
...
<div class="topBox">
    ...
    <tr>
          ...
          <td height="22" valign="top"><div class="Righttop">
            ...
            <a href="#" target="content"><span id="Affiche"><?php echo $news?></span></a>
            </marquee>
            </div></td>
        </tr>
    <tr>
          <td height="41" valign="top"><div class="Rightbottom">
			  ...
			  <?php //判断是不是管理员 如果不是则判断有没有权限查看
			   if($LoginId==89){
                 echo " <li id=\"li1\" class=\"navBtn1\" onclick=\"SubonCK(g_index);\">即时注单</li>";
				  }else{
				 if($Users[0]['jishizhudan']=="on")
				 {
				  ...
				  <?php //判断是不是管理员 如果不是则判断有没有权限查看
			   if($LoginId==89){ 
                 echo " <li id=\"li3\" class=\"navBtn2\" onclick=\"SubonCK2(13);\">在线统计</li>";
				  }else{
