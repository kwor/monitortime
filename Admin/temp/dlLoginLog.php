                    ...
                    <tr>
                        ...
                        <!-- strat -->
                            <table border="0" cellspacing="0" class="conter">
                                ...
                                <?php if (!$result){echo '<td align="center" colspan="7">暫無記錄</td>';}else {
								$shijian='';
								$zzrr='';
                                ...
                                <tr align="center" onmouseover="this.style.backgroundColor='#FFFFA2'" onmouseout="this.style.backgroundColor=''">
                                	<td><?php echo $i+1?></td>
										<td>[<?php echo $result[$i]['g_name']; ?>]</td>
...
$xyde='不同';
if($result[$i]['g_ip']==$zzrr)
{
$xyde='相同';
}
echo $result[$i]['g_ip'].'['.$xyde.']';
$zzrr=$result[$i]['g_ip'];
//echo $zzrr;

}}else{echo $result[$i]['g_ip'];}?>
</td>									
                                    <td>
									<?php echo $result[$i]['g_ip_location'];?>
									</td>
									 <td>
									<?php echo $result[$i]['g_page'];?>
									</td>
									...
									$shangcizhi='';
									//获取所有记录循环显示
									$sql68 = "SELECT * FROM g_dl_log WHERE name = '{$result[$i]['g_name']}' and date>'{$shijian}' order by id desc";
                                    $result68 = $db->query($sql68, 1);
									for ($iii=0; $iii<count($result68); $iii++){
									if($result68[$iii]['page']!=$shangcizhi)
									{
									echo '<p>'.$result68[$iii]['page'].'-'.$result68[$iii]['date'].'</p>';
									}
									$shangcizhi=$result68[$iii]['page'];
									}
									//记录时间
									$shijian=$result[$i]['g_date'];
									?>
									</td>
                                </tr>
                               <?php }}?>
                    ...
                    <tr>
						...
						<td class="f" align="center">註意：登陸日誌最少被保畱7天、超過7天部分。</td>
