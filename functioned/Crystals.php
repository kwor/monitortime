	...
	if ($CentetArr['cryList'] != null )
	{
		...
		for ($i=0; $i<count($CentetArr['cryList']); $i++)
		{
			...
			for ($n=0; $n<count($CentetArr['cryList'][$i]['cry']); $n++)
			{
				...
				if ($CentetArr['cryList'][$i]['cry'][$n]['g_mingxi_1_str'] != null ){ //連碼
				...
				//會員退水
				$count[2][8] = (($CentetArr['cryList'][$i]['cry'][$n]['g_tueishui']) /100)*$count[2][0]; 
				//會員輸贏单笔 已经减去水钱
				$count[2][9] = $CentetArr['cryList'][$i]['cry'][$n]['g_win'] != 0 ? $CentetArr['cryList'][$i]['cry'][$n]['g_win'] - $count[2][8] : 0; 
                
				$count[2][7] =  ($CentetArr['cryList'][$i]['cry'][$n]['g_tueishui_1']) /100*$count[2][0]; //代理退水
				$count[2][6] =  ($CentetArr['cryList'][$i]['cry'][$n]['g_tueishui_2']) /100*$count[2][0]; //總理退水
				$count[2][5] =  ($CentetArr['cryList'][$i]['cry'][$n]['g_tueishui_3']) /100*$count[2][0]; //股理退水
				$count[2][10] =  ($CentetArr['cryList'][$i]['cry'][$n]['g_tueishui_4']) /100*$count[2][0]; //公司退水
				//$count[2][10] =  0; //公司退水
				//會員總下注金額
				$count[1][1] += $count[2][0];
				//會員輸贏
					...
					case 56 :  //分公司報表
						...
						if ($CentetArr['cryList'][$i]['cry'][$n]['g_mumber_type'] == 2 && $lid == $xid) 
						{
							...
							if ($count[2][9] != 0 ){
								...
								$count[1][11] += $x - $count[1][6]-$count[1][7]; //應付上級
							}
						} 
						else 
						{
	...
	if ($CentetArr['cryList'] != null )
	{
		...
		for ($i=0; $i<count($CentetArr['cryList']); $i++)
		{
			...
			for ($n=0; $n<count($CentetArr['cryList'][$i]['cry']); $n++)
			{
				...
				$count[2][6] =  ($CentetArr['cryList'][$i]['cry'][$n]['g_tueishui_2']) /100*$count[2][0]; //總理退水
				$count[2][5] =  ($CentetArr['cryList'][$i]['cry'][$n]['g_tueishui_3']) /100*$count[2][0]; //股理退水
				$count[2][10] =  ($CentetArr['cryList'][$i]['cry'][$n]['g_tueishui_4']) /100*$count[2][0]; //公司退水.错，应该是分公司退水，公司无需退水，
					...
					case 22 :  //分公司報表
						...
						if ($CentetArr['cryList'][$i]['cry'][$n]['g_mumber_type'] == 2 && $lid == $xid) 
						{
							...
							if ($count[2][9] != 0 ){
								...
								$count[1][10] += $count[2][0]*$count[2][4]; //貢獻上級
								//$count[1][11] +=  $zwin;
								$count[1][11] += $x*$count[2][4]; //應付上級，，分公司应付   $x ---是会员总赢输
								//dump($count[2]);
