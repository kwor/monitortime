...
WHERE {$date} AND `g_nid` = '{$user[0]['g_name']}' AND yincang < 1 AND `g_win` is not null {$g_type} ORDER BY g_date DESC {$page->limit} ";
...
WHERE {$date} AND `g_nid` = '{$user[0]['g_name']}' AND yincang < 1 AND `g_win` is not null {$g_type}";
