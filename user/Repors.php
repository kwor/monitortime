...
$sql = "SELECT `g_id` FROM `g_zhudan` WHERE {$date} AND `g_nid` = '{$user[0]['g_name']}' and yincang<1 AND `g_win`  is not null {$g_type}";
...
WHERE {$date} and yincang<1 AND `g_nid` = '{$user[0]['g_name']}' AND `g_win` is not null {$g_type} ORDER BY g_date DESC {$page->limit} ";
...
WHERE {$date} and yincang<1 AND `g_nid` = '{$user[0]['g_name']}' AND `g_win` is not null {$g_type}";
...
if ($results)
{
	for ($i=0; $i<count($results); $i++)
	{
		...
		$countTNum222 += $countMoney['TuiShui'];
...
<table border="0" cellpadding="0" cellspacing="1" class="t_list t_result" width="700">
        ...
        <tr class="t_td_text" align="center" onMouseOver="this.style.backgroundColor='#FFFFA2'" onMouseOut="this.style.backgroundColor=''">
			...
			<td align="right"><?php echo $SumNum['TuiShui']?>&nbsp;</td>
        	<td align="right">
			<?php if($SumNum['Win']>0){
			echo "<font color=\"red\" style=\"font-weight:bold\" >".is_Number($SumNum['Win'], 1)."</font>&nbsp;";
			}else{
			echo "<font color=\"#009100\" style=\"font-weight:bold\" >".is_Number($SumNum['Win'], 1)."</font>&nbsp;";
			}?>
			</td>
        </tr>
        <?php }}?>
        <tr align="center" class="t_td_odd_2">
            ...
            <td align="right"><b><?php echo number_format($countTNum)?></b></td>
			 <td align="right"><b><?php echo number_format($countTNum222)?></b></td>
            <td align="right"><b><?php echo number_format($countSNum,1)?></b></td>
        </tr>
        <tr class="Man_bottom">
            <td colspan="6" align="right"><?php $p = $page->diy_page()?><table width='100%' height='22' border='0' cellspacing='0' cellpadding='0' class="page_box"><tr><td align='left'>&nbsp;共&nbsp;<?php echo $p[0];?>&nbsp;筆註單</td><td align='center'>共&nbsp;<?php echo $p[2];?>&nbsp;頁</td><td align='right'>&nbsp;<?php echo $p[4];?>『<?php echo $p[5];?>』<?php echo $p[6];?>&nbsp;</td></tr></table></td>
             </tr>
</table>
</body>
</html>