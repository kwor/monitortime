...
if (isset($_SESSION['guid_code']))
{
	...
	if ($action == 'fn')
	{
		...
		$h = $_POST['hid'];
		$odds = GetOddsxyft ($types, $h); 														//賠率
		if ($types=='冠亞和' || $types == "冠、亞軍和"){
			$odds = setoddsxyft($h, $odds, $user, 1,$ty);
		} else {
			$odds = setoddsxyft($h, $odds, $user, 0,$ty);
		}
	...
	else if ($action == 'fn3') //雙面提交
	{
		...
		for ($i=0; $i<count($sm_arr); $i++){
			$odds = GetOddsxyft ($sm_arr[$i][0], $sm_arr[$i][1]); 														//賠率
			...
			$odds = setoddsxyft($hv, $odds, $user, 1,$ty);
			} else {
			$odds = setoddsxyft($hv, $odds, $user, 0,$ty);
			}
