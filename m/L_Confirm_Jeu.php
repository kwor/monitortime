...
if($gameid==0 || $gameid==1 || $gameid==9){
	...
	foreach($ball_list as $key=>$ball_num){
		...
		$max = GetUser_s_ot ($result, $user,$ball_name, $wf_name,$gameid2);
		...
		$ListArr[0]['g_qishu'] = $qishu;													//下注期數
		$ListArr[0]['g_mingxi_1'] = $ball_name;  													//明細1 例如：		第一球
		$ListArr[0]['g_mingxi_1_str'] = null;													//字符串明細_1 	標明連碼用的
		$ListArr[0]['g_mingxi_2'] = $wf_name;												//明細2 				下注號碼
		$ListArr[0]['g_mingxi_2_str'] = null;													//字符串明細_2 	標明連碼用的		
		$ListArr[0]['g_odds'] = $odds;															//賠率
		$ListArr[0]['g_jiner'] = $money;
		
		$DtnArray = SumRankDistribution ($user, $money, $wf_name, $ball_name,$gameid2);
		...
		if ($ListArr[0]['id'] == null) exit(back("抱歉！{$ListArr[0]['g_mingxi_1']}『{$ListArr[0]['g_mingxi_2']}』下註失敗"));
...
//===========================================江苏快三===========================================================================
	...
	foreach($ball_list as $key=>$ball_num){
		...
		if($h_list[$key]!='h7')
		   {
			...
			$c_arr = isSzType('Ball_'.$ball_num, $h_list[$key], true);
			//$c_arr=isBallTypecqsm('Ball_'.$ball_num, $h_list[$key], true);
			$c=$c_arr[0];
		}
		//back('非法提交,提交的玩法或投注号码错误'.$c);exit;
		//$c="點數";
		//print_r($c);exit;
		...
		//echo $c;
		//print_r($result);exit;
		$max = GetUser_s_js ($result, $user,$ball_name, $wf_name,$gameid);
		isUserMoney_wap ($money, $max,$all_money);
