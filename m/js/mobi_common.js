...
window.onpopstate = function (event) {
    ...
    if (state != 'null') {
        ...
        //$('#_type').valueOf();
        //var url = $('#_type option:selected').attr('url');
        //alert(location.href);

        //调用分析获取url地址参数
        var _type = getQueryStringV(location.href, "_type");
        var player_type = getQueryStringV(location.href, "player_type");
        $("#_type option[value=" + _type + "]").attr("selected", "selected");
...
function gotoPage() {//change彩种
    var  = $('#_type').val();
    var url = $('#_type option:selected').attr('url');
    window.location = url + "?_type=" + ;
}

function isGetOpenRequest() {//是否需要请求开奖结果
...
var open_timer = null;
function LoadOpenData() {//ajax 读取开奖和其他数据
    try {
        $.ajax({
            async: true,
            url: G.open_url,
            ...
            success: function (jsonTxt) {
                ...
                if (isGetOpenRequest()) {
                    ...
                    open_timer = setTimeout("LoadOpenData()", 3000);
                }
            }
        })
    } catch (e) { }
    ...
    try {
        $.ajax({
            ...
            success: function (jsonTxt) {
                ...
                if (isGetOpenRequest()) {
                    LoadOpenData(); //下注期数>开奖期数+1,请求开奖结果
                }
            }
        })
    } catch (e) { }
...
function ParseJsonTxt(data) {
    ...
    if (status == "3") { //未开盘
        ...
        window.location.href = "Closed.php?_type=" +  +"&player_type=" + player;
    }
    if (status == "1") {
        ...
        if (data.k_open_time) {//开奖时间
            SetDataOpenTime(data.k_open_time);
        }
...
function SetDataPaiHang(title, content) {
    ...
    for (var i = 0; i < len; i++) {//<div class="paihang" ><dd><p>北</p></dd></div>
        ...
        var s = content[i]; //"16,1|03,1|04,2|02,1|04,1...3,1|08,1|06,1|05,1|03,1"
        var column = s.split("|");
...
function gotoPlayerPage() { //change玩法
	...
	if(player=='d2' || player=='d3' || player=='d4' || player=='d5' || player=='d6' || player=='d7' || player=='d8')
    	var url = name + '_d1.php' + '?_type=' +  + '&player_type=' + player;
	else
		var url = name + '_' + player + '.php' + '?_type=' +  + '&player_type=' + player;
    //console.log(url);
    window.location = url;
}