...
class SumAmountcq
{
	...
	private function GetNumberIsNull ()
	{
		...
		for ($i=0; $i<count($result); $i++)
		{
			...
			if ($result[$i]['g_result'] == '和')
			{
				$money = $result[$i]['g_jiner'];
				$result[$i]['g_win'] = 0;
			}
			else if ($result[$i]['g_result'] == '贏')
			{
			...
			if ($result[$i]['g_win'] > $ConfigModel['g_max_money'])
			{
	...
	private function Formula ()
	{
		...
		if ($numberList)
		{
			...
			$sql = "SELECT `yincang`, `g_id`, `g_s_nid`, `g_mumber_type`, `g_nid`, `g_date`, `g_type`, `g_qishu`, `g_mingxi_1`, `g_mingxi_1_str`, `g_mingxi_2`, `g_mingxi_2_str`, `g_odds`, `g_jiner`, `g_tueishui`, `g_tueishui_1`, `g_tueishui_2`, `g_tueishui_3`, `g_tueishui_4`, `g_distribution`, `g_distribution_1`, `g_distribution_2`, `g_distribution_3`, `g_win`, `g_t_id` ,`g_awin` ,`g_afail`
			...
			{
				if (Matchs::isNumber($resultList[$i]['g_mingxi_2'],1,1) )
				{
