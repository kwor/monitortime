...
class UserModel 
{
	...
	public function GetRankAlldl($gnid)
	{
		$sql = "SELECT `g_nid`, `g_name` FROM g_rank WHERE g_nid like'{$gnid}%' ORDER BY g_date DESC";
		$rankResult = $this->db->query($sql, 1);
		$list = array();
		if ($rankResult)
		{
	...
	public function GetMemberAlldl($gnid)
	{
		$sql = "SELECT `g_name` FROM g_user WHERE g_nid like'{$gnid}%' ORDER BY g_date DESC";
		return $this->db->query($sql, 0);
	}
	...
	public function SumMoney ($nid, $param=FALSE)
	{
		...
		if (!$param ) { //取非會員級別的總額
			...
			$sql1 = "SELECT SUM(g_up_value-g_initial_value) FROM `g_insert_log` WHERE g_up_type='余额' and g_nid LIKE '{$nid}' LIMIT 1";
			$rr = $this->db->query($sql1, 0);
			$rz = $rr[0][0];
			$r = $r + $n+$rz;
	...
	public function AddUser ($userList)
	{
		...
		`g_distribution`, `g_distribution_limit`, `g_Immediate_lock`, `g_lock`, `g_ip`, `g_date`, `g_uid`,`g_zcgs`,`g_zy`,`g_panlus`) VALUES(
		'{$userList['g_nid']}',
		'{$userList['g_login_id']}',
		'{$userList['g_name']}',
		'{$userList['g_password']}',
	...
	{
		$sql = "INSERT INTO g_user (g_nid, g_login_id, g_name, g_f_name, g_mumber_type, g_password, g_money, g_money_yes, g_distribution, g_panlus,g_panlu, g_xianer, g_out, g_look, g_ip, g_date, g_uid, money_dx,g_leixing) VALUES (
		'{$userList['g_nid']}',
		'{$userList['g_login_id']}',
		'{$userList['g_name']}',
		'{$userList['g_f_name']}',
		...
		for ($i=0; $i<count($result); $i++)
		{
			$sql.= "('{$userList['g_name']}', '{$result[$i][0]}', '{$result[$i][1]}', '{$result[$i][2]}', '{$result[$i][3]}'"; 			         
			for($m=1;$m<=$lenght;$m++){
			$flag=3+$m;
		//	$sql.=",'{$result[$i][$flag]}'";
		
		if($result[$i][$flag]-$userList['g_tuishui'] < 0 ) {
		$sql.=",'0'";
		}
		if($result[$i][$flag]-$userList['g_tuishui'] >= 0 ) {
		...
		//exit(alert_href($userList['g_tuishui'].'-'.$result[$i][$flag]));
			}
...
// 如果$_SERVER有值
	...
	public static function GetNextRank ($cid, $LoginId, $Users)
	{
		...
		else if ($cid == 2 && ($LoginId ==89 || $LoginId ==56) ) //新增股東
		{
			if ($LoginId ==89 ){
		...
		else if ($cid == 3 && ($LoginId ==89 || $LoginId ==56 || $LoginId ==22) ) //新增總代理
		{
			if ($LoginId ==89 ){
		...
		else if ($cid == 4 && ($LoginId ==89 || $LoginId ==56 || $LoginId ==22 || $LoginId ==78) ) //新增代理
		{
			if ($LoginId ==89 ){
		...
		else if ($cid == 5 && ($LoginId ==89 || $LoginId ==56 || $LoginId ==22 || $LoginId ==78 || $LoginId ==48) ) //新增會員
		{
			if ($LoginId ==89 ){
