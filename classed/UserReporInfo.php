...
class UserReportInfo 
{
	...
	private function GroupCount ($RepList, $typeString, $number=null)
	{
		...
		for ($i=0; $i<count($RepList); $i++)
		{
			...
			if ($RepList[$i]['g_mingxi_1'] == '第一球' ){
				$arr['count_c'][1] += $RepList[$i]['g_jiner'];
				$arr['count_d'][0] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第二球' ){
				$arr['count_c'][2] += $RepList[$i]['g_jiner'];
				$arr['count_d'][1] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第三球' ){
				$arr['count_c'][3] += $RepList[$i]['g_jiner'];
				$arr['count_d'][2] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第四球' ){
				$arr['count_c'][4] += $RepList[$i]['g_jiner'];
				$arr['count_d'][3] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第五球' ){
				$arr['count_c'][5] += $RepList[$i]['g_jiner'];
				$arr['count_d'][4] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第六球' ){
				$arr['count_c'][6] += $RepList[$i]['g_jiner'];
				$arr['count_d'][5] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第七球' ){
				$arr['count_c'][7] += $RepList[$i]['g_jiner'];
				$arr['count_d'][6] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第八球' ){
				$arr['count_c'][8] += $RepList[$i]['g_jiner'];
				$arr['count_d'][7] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '總和、龍虎' ){
			...
			} else if ($RepList[$i]['g_mingxi_1'] == '任選二' ){
				$arr['count_c'][13] += $RepList[$i]['g_jiner'];
				$arr['count_d'][12] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '選二連直' ){
				$arr['count_c'][14] += $RepList[$i]['g_jiner'];
				$arr['count_d'][13] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '選二連組' ){
				$arr['count_c'][15] += $RepList[$i]['g_jiner'];
				$arr['count_d'][14] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '任選三' ){
				$arr['count_c'][16] += $RepList[$i]['g_jiner'];
				$arr['count_d'][15] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '選三連直' ){
				$arr['count_c'][17] += $RepList[$i]['g_jiner'];
				$arr['count_d'][16] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '選三連組' ){
				$arr['count_c'][18] += $RepList[$i]['g_jiner'];
				$arr['count_d'][17] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '任選四' ){
			...
			if ($RepList[$i]['g_mingxi_1'] == $typeString ) 
			{
	...
	private function GetUserCrystals($UserList, $sArr, $arr, $typeString)
	{
		...
		for ($i=0; $i<count($UserList); $i++){
			...
			if ($UserList[$i]['g_mingxi_1'] == $typeString ) 
			{
...
/**
	...
	private function SumReport ($result, $logId)
	{
		...
		for ($i=0; $i<count($result); $i++) 
		{
			...
			if ($logId == 89 ){
				//注額計算，注額*佔成
				$ts = ((($result[$i]['g_tueishui_4'])/100) * $result[$i]['g_jiner']) * ($result[$i]['g_distribution_4']/100);
				$List[$i]['g_jiner'] = $result[$i]['g_jiner'] * ($result[$i]['g_distribution_4']/100);
			}else if($logId == 56){
				if ($result[$i]['g_tueishui_3'] >0 ){
					$ts = ((($result[$i]['g_tueishui_3'])/100) * $result[$i]['g_jiner']) * ($result[$i]['g_distribution_3']/100);
				} else {
					$ts = ((($result[$i]['g_tueishui'])/100) * $result[$i]['g_jiner']) * ($result[$i]['g_distribution_3']/100);
				}
				$List[$i]['g_jiner'] = $result[$i]['g_jiner'] * ($result[$i]['g_distribution_3']/100);
			}else if ($logId == 22){
				//注額計算，注額*佔成
				if ($result[$i]['g_tueishui_2'] >0 ){
					$ts = ((($result[$i]['g_tueishui_2'])/100) * $result[$i]['g_jiner']) * ($result[$i]['g_distribution_2']/100);
				} else {
					$ts = ((($result[$i]['g_tueishui'])/100) * $result[$i]['g_jiner']) * ($result[$i]['g_distribution_2']/100);
				}
				$List[$i]['g_jiner'] = $result[$i]['g_jiner'] * ($result[$i]['g_distribution_2']/100);
			}else if ($logId == 78){
				//注額計算，注額*佔成
				if ($result[$i]['g_tueishui_1'] >0 ){
					$ts = ((($result[$i]['g_tueishui_1'])/100) * $result[$i]['g_jiner']) * ($result[$i]['g_distribution_1']/100);
				} else {
					$ts = ((($result[$i]['g_tueishui'])/100) * $result[$i]['g_jiner']) * ($result[$i]['g_distribution_1']/100);
				}
				$List[$i]['g_jiner'] = $result[$i]['g_jiner'] * ($result[$i]['g_distribution_1']/100);
			}else if ($logId == 48){
				//注額計算，注額*佔成
				$ts = ((($result[$i]['g_tueishui'])/100) * $result[$i]['g_jiner']) * ($result[$i]['g_distribution']/100);
