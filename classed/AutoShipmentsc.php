...
class AutoLet 
{
	...
	private function InsertCry($result)
	{
		...
		if ($Users )
		{
			...
			if ($yes_money > 0 && $yes_money > $s_money )
			{
	...
	private function MyCry($userList, $cryList)
	{
		...
		$arral = array();
		if ($bMoney > 0 && stristr($cryList['g_s_nid'], $userList['g_nid']) )
		{
	...
	private function SumReport ($result, $logId)
	{
		...
		if ($logId == 89 || $logId == 56){
			//注額計算，注額*佔成
			$ts = ((($result['g_tueishui_3'])/100) * $result['g_jiner']) * ($result['g_distribution_3']/100);
			$List['g_jiner'] = $result['g_jiner'] * ($result['g_distribution_3']/100);
		}else if ($logId == 22){
			//注額計算，注額*佔成
			if ($result['g_tueishui_2'] >0){
				$ts = ((($result['g_tueishui_2'])/100) * $result['g_jiner']) * ($result['g_distribution_2']/100);
			} else {
				$ts = ((($result['g_tueishui'])/100) * $result['g_jiner']) * ($result['g_distribution_2']/100);
			}
			$List['g_jiner'] = $result['g_jiner'] * ($result['g_distribution_2']/100);
		}else if ($logId == 78){
			//注額計算，注額*佔成
			if ($result['g_tueishui_1'] >0){
				$ts = ((($result['g_tueishui_1'])/100) * $result['g_jiner']) * ($result['g_distribution_1']/100);
			} else {
				$ts = ((($result['g_tueishui'])/100) * $result['g_jiner']) * ($result['g_distribution_1']/100);
			}
			$List['g_jiner'] = $result['g_jiner'] * ($result['g_distribution_1']/100);
		}else if ($logId == 48){
			//注額計算，注額*佔成
			$ts = ((($result['g_tueishui'])/100) * $result['g_jiner']) * ($result['g_distribution']/100);
	...
	private function GetAutoLetList()
	{
		...
		if ($result && $this->ConfigModel['g_automatic_bu_huo_lock'] == 1 )
		{
			$List = array();
