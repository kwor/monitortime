...
class UserReportInfopk
{
	...
	private function GroupCount ($RepList, $typeString, $number=null)
	{
		...
		for ($i=0; $i<count($RepList); $i++)
		{
			...
			if ($RepList[$i]['g_mingxi_1'] == '冠军' ){
				$arr['count_c'][1] += $RepList[$i]['g_jiner'];
				$arr['count_d'][0] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '亚军' ){
				$arr['count_c'][2] += $RepList[$i]['g_jiner'];
				$arr['count_d'][1] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第三名' ){
				$arr['count_c'][3] += $RepList[$i]['g_jiner'];
				$arr['count_d'][2] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第四名' ){
				$arr['count_c'][4] += $RepList[$i]['g_jiner'];
				$arr['count_d'][3] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第五名' ){
				$arr['count_c'][5] += $RepList[$i]['g_jiner'];
				$arr['count_d'][4] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第六名' ){
				$arr['count_c'][6] += $RepList[$i]['g_jiner'];
				$arr['count_d'][5] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第七名' ){
				$arr['count_c'][7] += $RepList[$i]['g_jiner'];
				$arr['count_d'][6] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第八名' ){
				$arr['count_c'][8] += $RepList[$i]['g_jiner'];
				$arr['count_d'][7] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第九名' ){
				$arr['count_c'][9] += $RepList[$i]['g_jiner'];
				$arr['count_d'][8] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '第十名' ){
				$arr['count_c'][10] += $RepList[$i]['g_jiner'];
				$arr['count_d'][9] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '冠亞和' ){
			...
			} else if ($RepList[$i]['g_mingxi_1'] == '冠、亞軍和' ){
			...
			if ($RepList[$i]['g_mingxi_1'] == $typeString[0] || $RepList[$i]['g_mingxi_1'] == $typeString[1] ||$RepList[$i]['g_mingxi_1'] == $typeString[2] || $RepList[$i]['g_mingxi_1'] == $typeString[3] ) 
			{
			switch($this->cid){
	...
	private function GetUserCrystals($UserList, $sArr, $arr, $typeString)
	{
		...
		for ($i=0; $i<count($UserList); $i++){
			...
			} else if ($UserList[$i]['g_mingxi_1'] == '冠、亞軍和' ){
				$arr['count_c'][11] = $arr['count_c'][13]-$n*$h;
				//$arr['count_d'][12] ++;
			}
			
			if ($UserList[$i]['g_mingxi_1'] == $typeString[0] || $UserList[$i]['g_mingxi_1'] || $UserList[$i]['g_mingxi_1'] == $typeString[2] || $UserList[$i]['g_mingxi_1'] == $typeString[3] ) 
			{
			switch($this->cid){
				case 1:$tempnum=0;break;
				case 2:$tempnum=2;break;
			...
			if( $this->cid==1){
			if ($UserList[$i]['g_mingxi_1'] == "冠、亞軍和" ) 
			{
				switch ($UserList[$i]['g_mingxi_2']) {
					case '3' : 
